...
public:
  ...
  void readFrom(int fd)
  {
    ...
    {
      total_length_ += ret;
      CONSOLE_BRIDGE_logWarn("Read %d bytes from fd, total length is %d.", ret, total_length_);
    }
    else
    {
  ...
  char* getNextBuffer()
  {
    if (total_length_ == 0)
    {
      ...
      CONSOLE_BRIDGE_logWarn("Empty buffer, nothing to return.");
      return NULL;
    }
    ...
    if (end_of_first_message_ == NULL)
    {
      ...
      CONSOLE_BRIDGE_logWarn("No ETX found, nothing to return.");
      return NULL;
    }
